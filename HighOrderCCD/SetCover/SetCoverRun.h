...
public:
    ...
    static set_data create_random_set_data(int set_size, int elment_count) {
        int n, m;
        m = set_size;
        n = elment_count;
        set_data ret_struct;
    ...
    static bool validate_result(std::list<set_*>* ret, unsigned int uniqu_element_count) {
        std::list<set_*>::const_iterator iterator;
        unsigned int result_element_count = 0;
        for (iterator = ret->begin(); iterator != ret->end(); ++iterator) {
            set_::element* element = (*iterator)->elements;
            do {
    ...
    static set_data read_in_set_data(char* dbname) {
        // Open the stream
    ...
    static void set_cover_run()
    {

        std::cout << "start readin";
        std::cout.flush();
        set_data set_data = read_in_set_data("set_cover.txt");
        //set_data set_data = create_random_set_data(5000,5000);
        ...
        std::list<set_*> ret = setcover.execute_set_cover();
        std::cout << " --- Done" << std::endl;
        ...
        std::list<set_*>::const_iterator iterator;
        ofstream ios_("set_cover_result.txt");
        for (iterator = ret.begin(); iterator != ret.end(); ++iterator) 
        {
            ios_ << (*iterator)->set_id - 1 << std::endl;
